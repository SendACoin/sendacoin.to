...
const boldFont = fetch(new URL('../../../public/fonts/DMSans-Bold.ttf', import.meta.url).toString()).then((res) =>
	res.arrayBuffer()
);

const mediumFont = fetch(new URL('../../../public/fonts/DMSans-Medium.ttf', import.meta.url).toString()).then((res) =>
	res.arrayBuffer()
);
...
export default async function ImageHandler(req: NextRequest) {
	...
	const randomLinearGradient = () => {
		...
		const randomNumberGradient1 = () => Math.floor(Math.random() * (100 - 45 + 1) + 45);
		const randomNumberGradient2 = () => Math.floor(Math.random() * (44 - 0 + 1) + 0);
	...
	return new ImageResponse(
		(
			<div
				...
				style={{
					// backgroundImage: 'radial-gradient(circle at 25% 25%, #141318 0%, #fff 15%)',
			...
			>
				<div tw="flex w-full justify-between px-11">
					<img
						width="120"
						height="120"
						src="https://pbs.twimg.com/profile_images/1559888911660306432/uuRqi5f__400x400.png"
						tw="rounded-full"
						alt="Railly Hugo"
					/>
				</div>
